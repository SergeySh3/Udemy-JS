...
function calculator() {
  ...
  let sex, height, weight, age, ratio; // сохранение выбранных значений в localStorage и значения по умолчанию
  ...
  initLocalSettings('.calculating__choose_big div', 'calculating__choose-item_active'); // основная функция с формулой подсчета
  ...
  calcTotal(); // функция по собиранию статичных данных (пол и коэфициент активности(1 и 3 блок)) 

  function getStaticInformation(selector, activeClass) {
    const elements = document.querySelectorAll(selector); // вешаем обработчик, используем делегирование
    ...
    elements.forEach(elem => {
      elem.addEventListener('click', e => {
        if (e.target.getAttribute('data-ratio')) {
          ...
          // если нет дата-атрибута data-ratio, значит это пол. Записываем в sex id из e.target
  ...
  getStaticInformation('.calculating__choose_big div', 'calculating__choose-item_active'); // функция по собиранию динамических данных (вес, рост, возраст)

  function getDynamicInformation(selector) {
    const input = document.querySelector(selector); //вешаем обработчик на каждый инпут
...
function cards() {
  ...
  class MenuCard {
    ...
    render() {
      const element = document.createElement('div'); // Добавляем классы из rest-опертора (...classes)
...
function forms(formSelector, modalTimerId) {
  ...
  const forms = document.querySelectorAll(formSelector); // объект с сообщениями    
  ...
  function bindPostData(form) {
    form.addEventListener('submit', e => {
      ...
      const json = JSON.stringify(Object.fromEntries(formData.entries())); // fetch-запрос на сервер
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  ...
  const modalTrigger = document.querySelectorAll(triggerSelector),
        modal = document.querySelector(modalSelector); // Вешаем на кнопки обработчик с открытием модального окна
...
function slider({
  ...
  let slideindex = 1;
  let offset = 0; // Подставляем в total и current общее кол-во слайдов
  ...
  slidesField.style.transition = '0.5s all'; // Скрываем все лишние слайды, видимы только активный

  slidesWrapper.style.overflow = 'hidden'; // Назначаем всем слайдам одинаковую ширину
  ...
  slider.append(indicators); //добавдяем точки на страницу и делаем первую точку непрозрачной
  ...
  next.addEventListener('click', () => {
    if (offset == deleteNotDigits(width) * (slides.length - 1)) {
      // случай когда долистываем до последнего слайда
  ...
  prev.addEventListener('click', () => {
    if (offset == 0) {
      // случай когда стоит первый слайд          
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  const tabs = document.querySelectorAll(tabsSelector),
        ...
        tabsParent = document.querySelector(tabsParentSelector); //Скрыть картинку и текст в табе      
...
/* harmony export */   "getRecource": function() { return /* binding */ getRecource; }
/* harmony export */ });
...
async function getRecource(url) {
  let res = await fetch(url); // Если не получится сделать fetch-запрос, создаем новую ошибку
